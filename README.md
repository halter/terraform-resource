...
* `lock_timeout`: *Optional. Default `300s`* Duration to retry a state lock. Applies to every `put` against this resource. Note this default differs from Terraform's own `0s`, which gives up immediately when another operation holds the lock. Set `0s` explicitly to restore that behaviour. See the [Terraform docs](https://www.terraform.io/cli/commands/apply#lock-timeout-duration) for more information.
...
* `lock_timeout`: *Optional. Default `300s`* Duration to retry a state lock. See description under `source.lock_timeout`.
